<template>
  <div class="min-h-screen" style="background-color: #FFF8F0; font-family: 'Noto Sans TC', sans-serif;">
    ...
    <!-- Hero Banner -->
    <HeroBanner v-model:search="searchQuery" @search="handleSearch" />
...
// filteredCats 改用 activeSearch
const filteredCats = computed(() => {
  let list = cats.value
  if (activeFilter.value !== 'all') {
    list = list.filter((cat: Cat) => cat.status === activeFilter.value)
  }
  if (activeSearch.value.trim()) {  // 這裡改
    const q = activeSearch.value.toLowerCase()  // 這裡改
